...
results = reactive({
  ...
  r <<- range(res$time) ## axis range
  blas = get_option(input$blas)
  if(!is.null(blas)) {
    res = res[res$blas_optimize == blas,]
  }
  byte = get_option(input$byte)
  if(!is.null(byte)) {
    res = res[res$byte_optimize == byte,]
  }
  ...
  if(!is.null(input$results)) {
     ret = paste(ret, "File uploaded.", br(), br())
      ...
      if(sum(user_rank) > 0) {
        ...
        ret = paste(ret, br(), "For your current selection, your own
...
renderPlotly({
  ...
  res$colour = "black"
  res$color[res$is_user] = "steelblue"
  plot_ly(res, x = ~Rank, y = ~`Time (sec)`, type="scatter",
  marker = list(color = ~color, width=10, size=~size),           
...
#barplot(as.vector(tab), names.arg = 2^n)
  plot_ly(
    x = x_axis,
    y = ram,
    name = "CPU distribution",
